...
function getUTCMidnightTimestamp(year: number, month: number, date: number): number {
    const utcTimestamp = Date.UTC(year, month, date,
        0, // Hours
        0, // Minutes
        0, // Seconds
        0, // Milliseconds
...
function getTodayMidnightTimestamp(): number {
    ...
    return getUTCMidnightTimestamp(currentDate.getUTCFullYear(), currentDate.getUTCMonth(), currentDate.getUTCDate());
}
...
export async function setMidnightPairPrice() {
    ...
    for (const pair of pairs) {
        ...
        if (!data) {
            logger.error(`[ERROR][setMidnightPairPrice] Invalid pair price (${pairId}, ${priceFeedId})`);
            continue;
        }
        data['name'] = pair.name;
