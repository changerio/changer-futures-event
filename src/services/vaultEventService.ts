...
export async function getVaultEventUserData(address: string, startDateStr = '2023-08-01', endDateStr = '2023-10-26') {
    ...
    const { staking1Deposit, staking2Deposit } = calcStakingDeposit(deepCopy(dailyDeposit), startDateStr, endDateStr);

    return { staking1Deposit, staking2Deposit, dailyDeposit };
}

export async function getVaultEventUserList(startDateStr = '2023-08-01', endDateStr = '2023-10-26', isCsv = false) {
    const ret = {};
...
export async function getVaultDepositUserList(startDateStr = '2023-08-01', endDateStr = '2023-10-26') {
    ...
    const depositUsers = traders.filter((trader) => trader?.receiveShares?.length > 0); // traders.slice(0,3); 
...
function calcStakingDeposit(dailyDepositData, startDateStr: string, endDateStr: string) {
    const endDate = new Date(endDateStr);
